...
async def _ensure_columns():
    ...
    async with get_db() as conn:
        # PostgreSQL не поддерживает ALTER TABLE ADD COLUMN IF NOT EXISTS в одной команде
        # Поэтому пробуем добавить каждую колонку и игнорируем ошибки
        alters = [
...
async def get_user_by_id(user_id: int) -> Optional[Tuple[int, str, str, str]]:
    async with get_db() as conn:
        row = await conn.fetchrow("SELECT user_id, full_name, username, nickname FROM users WHERE user_id = $1",
                                  user_id)
        return tuple(row) if row else None
...
async def get_user_by_nickname(nickname: str) -> Optional[Tuple[int, str, str, str]]:
    async with get_db() as conn:
        row = await conn.fetchrow("SELECT user_id, full_name, username, nickname FROM users WHERE nickname = $1",
                                  nickname)
        if row: return tuple(row)
        row = await conn.fetchrow("SELECT user_id, full_name, username, nickname FROM users WHERE full_name = $1",
                                  nickname)
        if row: return tuple(row)
        row = await conn.fetchrow("SELECT user_id, full_name, username, nickname FROM users WHERE nickname LIKE $1",
                                  f"%{nickname}%")
        return tuple(row) if row else None
...
async def count_ontime_players_for_date(date_str: str) -> int:
    async with get_db() as conn:
        row = await conn.fetchrow("SELECT COUNT(*) FROM evening_booking WHERE date = $1 AND status = 'Вовремя'",
                                  date_str)
        return row[0] if row else 0
...
async def add_user_payment(user_id: int, amount: int):
    async with get_db() as conn:
        await conn.execute("UPDATE users SET total_paid = COALESCE(total_paid, 0) + $1 WHERE user_id = $2", amount,
                           user_id)
...
async def archive_current_evening():
    async with get_db() as conn:
        rows = await conn.fetch(
            "SELECT e.user_id, e.date, e.status, u.full_name, u.nickname FROM evening_booking e LEFT JOIN users u ON e.user_id = u.user_id")
        if rows:
            for row in rows:
                await conn.execute(
                    "INSERT INTO evening_history (date, user_id, status, full_name, nickname, amount) VALUES ($1, $2, $3, $4, $5, 0)",
                    row[1], row[0], row[2], row[3], row[4])
                if row[2] in ("Вовремя", "Позже"):
                    await conn.execute("UPDATE users SET last_visit = $1 WHERE user_id = $2", f"{row[1]} 20:00", row[0])
        await conn.execute("DELETE FROM evening_booking")
...
async def get_evenings_list(limit: int = 10) -> list:
    ...
    async with get_db() as conn:
        rows = await conn.fetch(
            f"SELECT date, COUNT(*) FROM evening_history WHERE status IN ('Вовремя', 'Позже') AND nickname NOT IN {orgs} AND full_name NOT IN {orgs} GROUP BY date ORDER BY id DESC LIMIT $1",
            limit)
        return [tuple(row) for row in rows]
...
async def get_user_extra_stats(user_id: int) -> Dict[str, float]:
    async with get_db() as conn:
        row = await conn.fetchrow(
            ...
            user_id)
    if not row:
        return {"avg_lh": 0.0, "removed_count": 0, "techfouls_total": 0, "ppk_guilty_count": 0, "pu_count": 0,
                "fouls_total": 0}
    ...
    async with get_db() as conn:
        for slot_num, s in slots.items():
            ...
            """,
                               ...
                               float(s.get('will_protocol_points', 0) or 0),
                               float(s.get('will_opinion_points', 0) or 0),
...
async def get_night_kills_order(game_date: str, game_number: int) -> List[int]:
    search_date = _ensure_iso_date(game_date)
    async with get_db() as conn:
        row = await conn.fetchrow("SELECT kill_order FROM night_kills_order WHERE game_date = $1 AND game_number = $2",
                                  search_date, game_number)
        if row and row[0]:
...
async def update_elo(user_id: int, delta: int):
    ...
    async with get_db() as conn:
        ...
        if row:
            await conn.execute("UPDATE elo_ratings SET elo = elo + $1, updated_at = NOW() WHERE user_id = $2", delta,
                               user_id)
        else:
...
async def add_user_achievement(user_id: int, achievement_id: str):
    async with get_db() as conn:
        await conn.execute(
            "INSERT INTO user_achievements (user_id, achievement_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", user_id,
            achievement_id)
...
async def add_tokens(user_id: int, amount: int, comment: str = "Игровые жетоны"):
    async with get_db() as conn:
        ...
        await conn.execute("INSERT INTO transactions (user_id, amount, type, comment) VALUES ($1, $2, 'tokens', $3)",
...
async def resolve_bet(bet_id: int, winner_team: str):
    async with get_db() as conn:
        await conn.execute("UPDATE bets_active SET winner_team = $1, resolved = TRUE WHERE id = $2", winner_team,
                           bet_id)
...
async def place_bet(user_id: int, bet_id: int, amount: int, predicted_winner: str) -> bool:
    try:
        async with get_db() as conn:
            await conn.execute(
                "INSERT INTO user_bets (user_id, bet_id, amount, predicted_winner) VALUES ($1, $2, $3, $4)",
